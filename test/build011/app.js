// TSOC Exercise v2.1.0 / Build018 - unified exercise / output language model
(async()=>{
...
function buildUnifiedExercises(){const list=JP_EXERCISES.map(jp=>{const st=languageStatus(jp),en=englishFor(jp.id);return st==="E"&&en?{...jp,name:en.name,purpose:en.purpose,description:en.description,language_status:st}:{...jp,language_status:st};});const only=langOverride("EX219")?.en?.name?langOverride("EX219").en:EN.english?.EN106;if(only)list.push({...((EX219_LOCAL)||{}),id:"EX219",display_id:"EX219",source_id:null,language_status:"E",name:only.name,purpose:only.purpose,description:only.description,english_id:"EN106",english:only,categories:EX219_LOCAL?.categories||[],images:EX219_LOCAL?.images||[],image_transforms:EX219_LOCAL?.image_transforms||[],print_images:EX219_LOCAL?.print_images||[],print_image_transforms:EX219_LOCAL?.print_image_transforms||[],qr:""});return list;}
DATA.exercises=buildUnifiedExercises();
/* Build018 #1: English QRの解決。①手動登録(TSOC_ENGLISH_QR_URLS) → ②static asset(assets/qr-en/exXXX.png)。
   静的assetは214件(日本語only 4件とEX219を除く)。EX219は手動登録QRのみを使い、staticは持たない(上書きしない)。 */
const EN_QR_STATIC_EXCLUDE=new Set(["EX065","EX122","EX149","EX150","EX219"]);
function resolveEnglishQr(id){
  const local=window.TSOC_ENGLISH_QR_URLS?.[id];
  if(local)return local;                                   // ①手動登録を最優先
  if(EN_QR_STATIC_EXCLUDE.has(id))return "";               // 日本語only4件・EX219はstaticなし
  return `assets/qr-en/${String(id).toLowerCase()}.png`;   // ②static asset
}
function getOutputExercise(e,lang){const status=languageStatus(e);if(lang==="ja")return status==="E"?null:e;const t=e.id==="EX219"?(langOverride("EX219")?.en?.name?langOverride("EX219").en:EN.english?.EN106):englishFor(e.id);return (status==="J"||!t)?null:{...e,name:t.name,purpose:t.purpose,description:t.description,qr:resolveEnglishQr(e.id)};}
...
function doseText(s){let t='';if(s.reps)t+=`${s.reps}${state.outputLanguage==="en"?(s.unit==="秒"?" seconds":" reps"):s.unit}`;if(s.sets)t+=(t?' × ':'')+`${s.sets}${state.outputLanguage==="en"?" sets":"セット"}`;return t}
