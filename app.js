...
function printImages(e){const set=getPrintImageSet(e);return set.images.length?`<div class="print-photo-layout${set.highRes?' high-res':''}">${set.images.map((src,i)=>`<div class="print-photo-item"><img src="${src}" alt="" style="${transformStyle(set.transforms?.[i])}"></div>`).join('')}</div>`:`<div class="print-no-photo">画像なし</div>`}
